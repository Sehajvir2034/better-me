...
import Link from "next/link";
import { MoonStar, Smile, Sunrise } from "lucide-react";
      ...
      <div className="group relative flex h-60 w-60 shrink-0 cursor-pointer flex-col justify-between overflow-hidden rounded-[2rem] bg-[#13151f] p-5 transition-all duration-300">
        ...
        <div className="relative p-0.5">
          ...
          <div className="mb-3 flex gap-2">
            <Badge
              variant="secondary"
              className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl py-3.75 text-xs font-bold tracking-wider shadow-none ${
                data.amDone
                  ? "  bg-rose-500/15 text-rose-300 hover:bg-rose-500/15"
                  : "  bg-white/5 text-white/35 hover:bg-white/5"
              }`}
            >
              <Sunrise size={13} className="shrink-0" />
              <span>AM</span>
              <span>{data.amDone ? "✓" : "—"}</span>
            </Badge>
            ...
            <Badge
              ...
              className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl py-3.75 text-xs font-bold tracking-wider shadow-none ${
                data.pmDone
                  ? "  bg-indigo-500/15 text-indigo-300 hover:bg-indigo-500/15"
                  : "  bg-white/5 text-white/35 hover:bg-white/5"
              }`}
            >
              <MoonStar size={13} className="shrink-0" />
              <span>PM</span>
              <span>{data.pmDone ? "✓" : "—"}</span>
            </Badge>
          </div>
